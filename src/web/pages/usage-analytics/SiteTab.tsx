...
import type { UsageBySiteResponse } from '../../api.js';
...
export default function SiteTab({
  ...
  const barSpec = useMemo(() => ({
    ...
    data: [{
      ...
      values: top10
        .map((d) => ({
          site: d.siteName.length > 20 ? d.siteName.slice(0, 20) + '...' : d.siteName,
    ...
    bar: {
      style: {
        ...
        fill: { gradient: 'linear' as const, x0: 0, y0: 0, x1: 1, y1: 0, stops: [{ offset: 0, color: CHART_COLORS[0] }, { offset: 1, color: '#818cf8' }] },
      },
    },
  ...
  if (items.length === 0) {
    return <EmptyState title="暂无站点使用数据" desc="所选时间范围内没有站点使用记录" />;
  }
      ...
      {isMobile ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {tableRows.map((item, index) => {
            const lat = formatLatency(item.avgLatencyMs);
            const rate = successRateBadge(item.successRate || 0);
            return (
              <MobileCard
                key={item.siteId}
                title={
              ...
              >
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  ...
                  <MobileField label="延迟" value={<span style={{ color: lat.color }}>{lat.text}</span>} />
                </div>
              </MobileCard>
            );
          })}
                ...
                return (
                  <tr key={item.siteId}>
                    ...
                    <td>
                      ...
                      {item.platform ? <span style={{ marginLeft: 6, fontSize: 11, color: 'var(--color-text-muted)' }}>{item.platform}</span> : null}
                    </td>
                    ...
                    <td style={{ textAlign: 'center' }}>
                      <span style={{ fontVariantNumeric: 'tabular-nums', fontSize: 12, fontWeight: 600, color: lat.color, background: lat.bg, padding: '2px 8px', borderRadius: 4 }}>{lat.text}</span>
                    </td>
                  </tr>
                );
              })}
      ...
      <div style={{ marginTop: 12, display: 'flex', justifyContent: 'flex-end' }}>
        <button
          ...
          onClick={() =>
            exportCsv(
              'usage-by-site.csv',
              ['站点', '平台', 'Tokens', '消耗', '调用', '成功调用', '成功率', '平均延迟(ms)'],
              items.map((d) => [d.siteName, d.platform || '', d.tokens, d.spend.toFixed(6), d.calls, d.successCalls, `${d.calls > 0 ? ((d.successCalls / d.calls) * 100).toFixed(1) : 0}%`, d.avgLatencyMs ?? '']),
            )
          }
        >
          导出 CSV
