...
import type { UsageByModelResponse } from '../../api.js';
...
export default function ModelTab({
  ...
  data: UsageByModelResponse | null;
  loading: boolean;
  isMobile: boolean;
}) {
  ...
  const pieSpec = useMemo(() => ({
    type: 'pie' as const,
    data: [{ id: 'data', values: top10.map((d) => ({ model: d.model, value: toSafeNumber(d.tokens) })) }],
    valueField: 'value',
    categoryField: 'model',
    outerRadius: 0.8,
    innerRadius: 0.55,
  ...
  if (items.length === 0) {
    return <EmptyState title="暂无模型使用数据" desc="所选时间范围内没有模型使用记录" />;
  }
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 14px', marginBottom: 16, padding: '0 4px' }}>
        {top10.map((d, idx) => (
          <span key={d.model} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--color-text-secondary)' }}>
            ...
            <ModelLabel model={d.model} maxLen={20} />
              ...
              <MobileCard
                ...
                title={
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <ModelLabel model={item.model} maxLen={30} />
                  </span>
                }
              >
              ...
              {tableRows.map((item, index) => {
                ...
                return (
                  <tr key={item.model}>
                    ...
                    <td><ModelLabel model={item.model} /></td>
      ...
      <div style={{ marginTop: 12, display: 'flex', justifyContent: 'flex-end' }}>
        <button
          ...
          onClick={() =>
            exportCsv(
              'usage-by-model.csv',
              ['模型', 'Tokens', '消耗', '调用', '成功调用', '成功率', '平均延迟(ms)'],
              items.map((d) => [d.model, d.tokens, d.spend.toFixed(6), d.calls, d.successCalls, `${d.successRate}%`, d.avgLatencyMs ?? '']),
            )
          }
        >
          导出 CSV
